...
export function TokenSaleProgress() {
  ...
  const infoCards = useMemo(() => {
    ...
    return [
      {
        title: "Tokens Sold",
        value: `${formatNumber(soldTokens)} IBCM`,
        info: "The total number of IBCM tokens that have been purchased so far."
      },
      {
        title: "Tokens Remaining",
        value: `${formatNumber(remainingTokens)} IBCM`,
        info: "The number of IBCM tokens still available for purchase."
      },
      {
        title: "Sale Progress",
      ...
      },
      {
        title: "Current Stage",
        value: currentStage,
        info: "The current stage of the IBCM token sale."
      },
      {
        title: "Current Price",
        value: `${currentPrice.toFixed(6)} USDC`,
        info: "The current price per IBCM token."
      },
      {
    ...
    <div className="px-4 sm:px-6 lg:px-8 pb-4 sm:pb-6 lg:pb-8">
      <Card className="overflow-hidden max-w-8xl mx-auto">
        <CardHeader className="bg-gradient-to-r from-[#e1d8c7] to-[#d1c8b7] text-gray-900 p-6">
          ...
          <p className="text-sm text-gray-700 mt-2">Track the progress of the IBCM token sale in real-time. Stay informed about the current stage, tokens sold, and remaining availability.</p>
        </CardHeader>
        <CardContent className="p-6 space-y-6">
          <div className="space-y-2">
            ...
            <div className="flex justify-between text-sm text-gray-600">
              <span>0 IBCM</span>
              <span>{formatNumber(saleInfo.totalTokens)} IBCM</span>
            </div>
          </div>
