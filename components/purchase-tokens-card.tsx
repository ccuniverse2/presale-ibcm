...
function PurchaseTokensCard() {
      ...
      (async () => {
        try {
          ...
          toast({
            ...
            description: `Successfully purchased ${amount} IBCM tokens. Transaction ID: ${signature}`,
          })
        } catch (error) {
  ...
  return (
    <Card className="pb-6">
      <CardHeader>
        <CardTitle>Purchase IBCM Tokens</CardTitle>
      </CardHeader>
      <CardContent>
      {!saleInfo ? (
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="space-y-2 mb-4">
            <div className="flex justify-between items-center bg-bark-secondary p-3 rounded-lg">
              <span className="text-sm font-medium text-gray-700">Price per IBCM:</span>
            ...
            <div className="flex justify-between items-center bg-bark-secondary p-3 rounded-lg">
              ...
              <span className="text-sm font-bold text-bark-primary">
                {formatNumber(saleInfo?.remainingTokens || 0)} - {formatNumber(TOKEN_SALE_CONFIG.maxPurchase)} IBCM
              </span>
            </div>
          </div>
          <div>
