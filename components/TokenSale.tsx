  ...
  const handleSubmit = useCallback(async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast({
        ...
        description: `Successfully purchased ${amount} IBCM tokens. Transaction ID: ${txid}`,
      })
    } catch (error) {
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      <div className="flex justify-between items-center">
        <div className="text-sm text-[#e1d8c7]">
          <p>Price: {saleInfo ? saleInfo.priceSOL.toFixed(6) : '...'} SOL per IBCM</p>
          <p>Available: {saleInfo ? formatNumber(saleInfo.remainingTokens) : '...'} IBCM</p>
        </div>
        <Image
