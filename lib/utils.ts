...
export function validatePurchase(amount: number): void {
  ...
  if (amount < minPurchase) {
    throw new Error(`Minimum purchase is ${minPurchase} IBCM tokens`)
  }
  if (amount > maxPurchase) {
    throw new Error(`Maximum purchase is ${maxPurchase} IBCM tokens`)
  }
...
export async function processPurchase(
  ...
  try {
    ...
    console.log(`Processed purchase of ${amount} IBCM tokens for ${cost} ${currency}. Transaction ID: ${txid}`);
    return txid;
  } catch (error) {
